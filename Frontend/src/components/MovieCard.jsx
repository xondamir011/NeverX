...
const T = {
  EN: { desc: "No description", details: "Details", save: "Save", saved: "Saved", like: "Like" },
  UZ: { desc: "Tavsif yo'q",    details: "Batafsil", save: "Saqlash", saved: "Saqlangan", like: "Like" },
  RU: { desc: "Нет описания",   details: "Подробнее", save: "Сохранить", saved: "Сохранено", like: "Лайк" },
  DE: { desc: "Keine Beschr.",  details: "Details",   save: "Speichern", saved: "Gespeichert", like: "Like" },
  TR: { desc: "Açıklama yok",   details: "Detay",     save: "Kaydet",    saved: "Kaydedildi",  like: "Beğen" },
};
...
export default function MovieCard({ movie, lang = "UZ" }) {
  const user = auth.currentUser; // ← faqat bir marta
  ...
  const loadLike = async () => {
    ...
    if (snap.exists()) {
      ...
      const userSnap = await getDoc(doc(db, "likes", String(movie.id), "users", user.uid));
  ...
  const loadSave = async () => {
    const all = await getSavedMovies();
    const found = all.find(m => m.movieId === movie.id);
  ...
  const toggleSave = async (e) => {
    ...
    try {
      if (saved && savedDocId) {
        ...
        setSaved(false); setSavedDocId(null);
      } else {
  ...
  // ── WATCH MOVIE (premium tekshiruvi) ──
  const watchMovie = (e) => {
    e.stopPropagation();
    if (!isPremium) {
    ...
    // Premium foydalanuvchi uchun — keyinroq video player qo'shiladi
    alert("🎬 Kino player tez kunda!");
  };
      ...
      <div onClick={openMovie}
        ...
        <div className="relative overflow-hidden aspect-[2/3]">
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
            <div className="bg-base-200/80 rounded-full w-14 h-14 flex items-center justify-center shadow-lg">
            ...
            <div className="p-4 sm:p-5">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                <button onClick={watchMovie}
                  className={`btn btn-sm gap-1 ${isPremium ? "btn-primary" : "bg-gradient-to-r from-blue-700 to-purple-600 text-white border-none"}`}>
                   {isPremium ? "Tomosha qilish" : "Tomosha qilish"}
                </button>
